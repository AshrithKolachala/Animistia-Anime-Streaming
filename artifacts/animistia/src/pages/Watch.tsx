...
export default function Watch() {
  ...
  useEffect(() => {
    if (params.season && seasonsQuery.data?.length) {
      const targetSeasonNum = Number(params.season);
      const targetSeasonObj = seasonsQuery.data.find((s) => s.seasonNumber === targetSeasonNum);
      if (targetSeasonObj) setSelectedSeasonId(targetSeasonObj.id);
    } else {
      if (!selectedSeasonId && seasonsQuery.data?.[0]) setSelectedSeasonId(seasonsQuery.data[0].id);
    }
  }, [seasonsQuery.data, selectedSeasonId, params.season]);

  useEffect(() => {
    if (currentEpisode) {
      setSelectedEpisodeId(currentEpisode.id);
    } else {
      if (!selectedEpisodeId && episodesQuery.data?.[0]) setSelectedEpisodeId(episodesQuery.data[0].id);
    }
  }, [episodesQuery.data, selectedEpisodeId, currentEpisode]);

  // Next episode handler passing clean routes down to custom player button
  const handleNextEpisode = useMemo(() => {
    if (!isSeries || !episodesQuery.data || !currentEpisode) return undefined;
    const currentIndex = episodesQuery.data.findIndex((ep) => ep.id === currentEpisode.id);
    const nextEpisodeObj = currentIndex !== -1 ? episodesQuery.data[currentIndex + 1] : null;

    if (nextEpisodeObj) {
      return () => {
        const slugName = show.title.toLowerCase().replace(/[^a-z0-9]+/g, '-');
        const activeSeasonNum = selectedSeason?.seasonNumber ?? 1;
        setLocation(`/watch/series/${slugName}/${activeSeasonNum}/${nextEpisodeObj.episodeNumber}`);
      };
    }
    return undefined;
  }, [isSeries, episodesQuery.data, currentEpisode, show.title, selectedSeason, setLocation]);
  ...
  const handleEpisodePickerClick = (episodeId: number) => {
    setSelectedEpisodeId(episodeId);
    const pickedEpisode = episodesQuery.data?.find((ep) => ep.id === episodeId);
    if (pickedEpisode) {
      const slugName = show.title.toLowerCase().replace(/[^a-z0-9]+/g, '-');
      const activeSeasonNum = selectedSeason?.seasonNumber ?? 1;
      setLocation(`/watch/series/${slugName}/${activeSeasonNum}/${pickedEpisode.episodeNumber}`);
    }
  };
  ...
  return (
    <Shell>
      ...
      <YouTubePlayer 
        ...
        episodeLabel={currentEpisode ? `Episode ${String(currentEpisode.episodeNumber).padStart(2, '0')} · ${currentEpisode.title}` : 'Feature presentation'} 
        captionsText={captionText} 
        onNextEpisode={handleNextEpisode}
        mediaType={show.mediaType} /* 🚀 PASSED THE SHOW MEDIA TYPE PROP HERE */
      />

              ) : storedVideoUrl ? (
                <video src={storedVideoUrl} controls className="h-full w-full" data-testid="video-uploaded-player" />
              ) : (
                <div className="relative flex h-full flex-col items-center justify-center overflow-hidden" style={{ background: 'radial-gradient(circle at 50% 38%, rgba(206,157,109,.24), transparent 27%), linear-gradient(135deg,#16151d,#27212d)' }}>
                  <div className="absolute h-[130%] w-px rotate-45 bg-primary/20" />
                  <div className="absolute h-[130%] w-px -rotate-45 bg-primary/20" />
                  <button onClick={() => setPlaying(!playing)} className="relative flex h-16 w-16 items-center justify-center rounded-full bg-primary text-primary-foreground transition hover:scale-105" aria-label={playing ? 'Pause preview' : 'Play preview'} data-testid="button-play-preview">
                    {playing ? <Pause size={22} fill="currentColor" /> : <Play size={22} fill="currentColor" />}
                  </button>
                  <span className="relative mt-4 font-mono-app text-[10px] uppercase tracking-[.2em] text-white/60">
                    {playing ? 'Preview playing' : isSeries && !currentEpisode ? 'Choose an episode' : 'Ready when you are'}
                  </span>
                </div>
              )}
            </div>
            <div className="mt-7 flex flex-col justify-between gap-4 sm:flex-row sm:items-start">
              ...
              <div className="flex gap-2">
                ...
                <button onClick={() => navigator.clipboard?.writeText(window.location.href)} className="flex h-9 w-9 items-center justify-center rounded-full border border-white/15 text-muted-foreground transition hover:border-primary hover:text-primary" aria-label="Copy show link" data-testid="button-share-show">
                  <Share2 size={14} />
                </button>
              </div>
            </div>
            <div className="mt-5 flex flex-wrap items-center gap-3 font-mono-app text-[10px] uppercase tracking-widest text-muted-foreground">
              <span>{show.year}</span>
              <span className="h-1 w-1 rounded-full bg-primary" />
              <span>{isSeries ? `${show.episodesCount} episodes` : 'Movie'}</span>
              <span className="h-1 w-1 rounded-full bg-primary" />
              <span className="flex items-center gap-1 text-primary">
                <Star size={11} fill="currentColor" />
                {typeof show?.rating === 'number' ? show.rating.toFixed(1) : '0.0'}
              </span>

              {show?.genres && Array.isArray(show.genres) ? (
                show.genres.map((genre) => (
                  <span key={genre} className="rounded border border-white/10 px-2 py-1 normal-case tracking-normal">
                    {genre}
                  </span>
                ))
              ) : (
                <span className="rounded border border-white/10 px-2 py-1 normal-case tracking-normal">Anime</span>
              )}

            </div>
            <p className="mt-6 max-w-2xl text-sm leading-7 text-muted-foreground">{currentEpisode?.synopsis || show.synopsis}</p>
            {isSeries && (
              <SeriesEpisodes 
          ...
          <aside>
            ...
            <div className="space-y-4">
              {related.map((item) => <RelatedCard key={item.id} show={item} />)}
            </div>
            <div className="mt-7 rounded-xl border border-white/10 bg-secondary/60 p-4">
              <div className="flex items-center gap-2 font-mono-app text-[10px] uppercase tracking-widest text-primary">
                <Clock3 size={13} /> A good evening
              </div>
              <p className="mt-3 text-xs leading-5 text-muted-foreground">No rush. Let the next story find its own pace.</p>
            </div>
          </aside>
        </div>
      </div>
  ...
  return (
    <section className="mt-10 rounded-2xl border border-white/10 bg-secondary/50 p-5 sm:p-6" data-testid="series-episode-picker">
      <div className="flex flex-col justify-between gap-4 sm:flex-row sm:items-center">
        <div>
          <div className="font-mono-app text-[10px] uppercase tracking-widest text-primary">Continue the story</div>
          <h2 className="mt-2 font-display text-2xl">Episodes.</h2>
        </div>
        {seasons.length > 0 && (
          <select value={selectedSeason?.id ?? ''} onChange={(event) => onSeasonChange(Number(event.target.value))} className="admin-input sm:max-w-[220px]" aria-label="Choose season">
            {seasons.map((season) => <option key={season.id} value={season.id}>Season {season.seasonNumber}{season.title ? ` · ${season.title}` : ''}</option>)}
          </select>
        )}
      </div>
      {loading ? (
        <p className="mt-5 text-sm text-muted-foreground">Loading episodes…</p>
      ) : episodes.length ? (
        ...
        <p className="mt-5 rounded-xl border border-dashed border-white/10 px-4 py-6 text-sm text-muted-foreground">
          {seasons.length ? 'No episodes have been published in this season yet.' : 'Episodes will appear here once the series is published.'}
        </p>
      )}
    </section>
  );
...
function RelatedCard({ show }: { show: import('@workspace/api-client-react').Show }) {
  const targetUrl = useMemo(() => {
    const slugName = show.title.toLowerCase().replace(/[^a-z0-9]+/g, '-');
    return show.mediaType === 'series' ? `/watch/series/${slugName}/1/1` : `/watch/movies/${slugName}`;
  }, [show.title, show.mediaType]);

  return (
    <Link href={targetUrl} className="group flex gap-3" data-testid={`link-related-${show.id}`}>
      <div className="relative h-20 w-28 shrink-0 overflow-hidden rounded-lg" style={{ background: 'linear-gradient(135deg,#252035,#bd796d)' }}>
        <span className="absolute bottom-2 left-2 font-display text-2xl text-white/75">{show.title.slice(0, 1)}</span>
        <div className="absolute inset-0 bg-black/10 transition group-hover:bg-transparent" />
      </div>
      <div className="min-w-0 py-1">
        <h3 className="truncate text-sm font-semibold text-foreground transition group-hover:text-primary">{show.title}</h3>
        <p className="mt-1 line-clamp-2 text-xs leading-4 text-muted-foreground">{show.genres.join(' · ')} / {show.year}</p>
      </div>
    </Link>
  );
}
